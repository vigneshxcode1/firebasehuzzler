...
//                 justifyContent: "center",
//                 alignItems: "center",
//                 gap: isMobile ? "8px" : "0px", // 🔥 mobile gap small
...
//             <div id="fh-header-left" className="fh-header-left">
//               <div id="fh-welcome" className="fh-welcome">
//                 <div id="fh-title" className="fh-title" >
//                   <h1 style={{ color: "#000", fontWeight: 400, fontSize: "36px" }}>Welcome,</h1>
//                   <h2 style={{ color: "#000", fontWeight: 400, fontSize: "36px" }}>{userInfo.firstName || "Huzzlers"}</h2>
//                 </div>
...
//                 >
//                   {/* background image */}
//                   <img src={Categories} alt={cat} className="category-bg-img" style={{ borderRadius: isMobile ? "18px" : "0px" }} />
...
import webAnalytics from "../services/Webanalyticsservice.jsx";
...
export default function ClientHomeUI() {
  ...
  // 🔥 INITIALIZE WEB ANALYTICS ON COMPONENT MOUNT
  useEffect(() => {
    const initAnalytics = async () => {
      const user = auth.currentUser;
      if (user && !analyticsInitialized.current) {
        try {
          await webAnalytics.init(user.uid);
          analyticsInitialized.current = true;
          console.log("✅ Analytics initialized for ClientHomeUI");
        } catch (error) {
  ...
  const fetchUserProfile = async (uid) => {
    try {
      ...
      console.error("Profile fetch error:", e);
      // 🔥 Track error
      webAnalytics.trackAppError(`Profile fetch failed: ${e.message}`);
    }
  };
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      try {
        ...
        console.error("Error fetching user:", err);
        // 🔥 Track error
        webAnalytics.trackAppError(`User fetch failed: ${err.message}`);
      }
    });
    return unsubscribe;
  }, []);
  ...
  async function acceptNotif(item) {
    // 🔥 Track notification acceptance
    webAnalytics.trackInteraction();
  ...
  async function openJob(job) {
    ...
    // 🔥 Track job card click
    webAnalytics.trackInteraction();

    const collectionName = job._source === "service_24h" ? "service_24h" : "services";
    ...
    try {
      ...
      console.error("Error updating views:", err);
      // 🔥 Track error
      webAnalytics.trackAppError(`View increment failed: ${err.message}`);
    }
