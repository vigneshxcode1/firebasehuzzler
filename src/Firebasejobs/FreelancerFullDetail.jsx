...
//   const [isRequested, setIsRequested] = useState(false);
//   const [isAccepted, setIsAccepted] = useState(false);
//   const [serviceRequests, setServiceRequests] = useState({}); // { [serviceId]: "sent" | "accepted" | "rejected" }
...
//         const data = doc.data();
//         if (data.jobId) requests[data.jobId] = data.status;
//         else requests["general"] = data.status;
//       });
...
//   // ---------------- Listen for collaboration request status ----------------
//   useEffect(() => {
//     if (!auth.currentUser?.uid || !uid) return;

//     // client = current user → freelancer = uid
//     const q = query(
//       collection(db, "collaboration_requests"),
//       where("clientId", "==", auth.currentUser.uid),
//       where("freelancerId", "==", uid)
...
//                     url: window.location.href,
//                   };
//                   if (navigator.share) navigator.share(shareData).catch(console.log);
//                   else {
//                     navigator.clipboard.writeText(shareData.url);
//                     alert("Profile URL copied to clipboard!");
//                   }
...
//             {(activeTab === "work" ? services : services24h).map((s) => {
//               const status = serviceRequests[s.id]; // per-service status
//               const generalStatus = serviceRequests["general"]; // general request status
//               const isDisabled = generalStatus === "sent" || generalStatus === "accepted";
...
//                     {status === "sent" && <button className="ffds-btn ffds-btn-disabled" style={{ marginTop: 8 }}>Requested</button>}
...
//                         className="ffds-btn ffds-btn-primary"
//                         style={{ marginTop: 8 }}
//                         disabled={isDisabled} // disabled if general request sent/accepted
//                         onClick={() => {
//                           setSelectedJob(s);
//                           openConnectPopup();
//                         }}
